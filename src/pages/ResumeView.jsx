...
export default function ResumeView() {
  ...
  const skillsWithUsage = skills.map((skill) => {
    const count = roleFilteredProjects.filter(
      (p) => Array.isArray(p.technologies) && p.technologies.includes(skill.name)
    ).length;
  ...
  }, [selectedSkill, roleFilteredProjects]);
    ...
    <div className="relative min-h-screen px-6 py-16 mx-auto overflow-hidden max-w-7xl">
      ...
      {role && (
        <>
            ...
            >
              {skillsWithUsage.map((skill) => (
                <motion.div
                  ...
                  className={`p-4 bg-white/60 backdrop-blur-lg border shadow-md rounded-xl cursor-pointer ${
                ...
                >
                  <SkillSlider {...skill} usageCount={skill.count} />
                </motion.div>
              ))}
            </motion.div>
          </section>
          ...
          />
<section className="py-20 text-center"> <h3 className="mb-6 text-4xl font-black">Want to explore my full portfolio?</h3> <p className="mb-6 text-gray-600"> My interactive resume highlights key skills and experience — and my full portfolio showcases detailed project work. </p> <a href="https://ishasaleemportfolio-c35mcy8mj-ishasaleems-projects.vercel.app/" target="_blank" rel="noopener noreferrer" className="inline-block px-10 py-4 text-xl font-bold text-white transition-transform rounded-full shadow-xl bg-gradient-to-r from-indigo-500 via-purple-500 to-pink-500 hover:scale-105" > View Full Portfolio 🚀 </a> </section>
        </>
